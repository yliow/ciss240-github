...
bool is_enemy_piece(char chessboard[8][8], char player, int r, int c);
bool is_valid_pawn_move(char chessboard[8][8],
                        char player,
...
bool is_enemy_piece(char chessboard[8][8], char player, int r, int c)
{
    ...
    return is_player_piece(chessboard, other_player, r, c);
}
...
bool is_valid_pawn_move(char chessboard[8][8],
                        ...
                        int r0, int c0, int r1, int c1)
{
    // direction: -1 means move pawn upward, +1 means move pawn downward
    int direction = (player == 'W' ? -1 : +1);
    int beginning_row = (player == 'W' ? 6 : 1);
    if (is_enemy_piece(chessboard, player, r1, c1))
    {
        // test capturing move
        return ((c1 == c0 - 1 || c1 == c0 + 1)
                && (r1 == r0 + direction * 1));
    }
    else
    {
        // test non-capturing move
        return ((r1 == r0 + direction * 1
                 || (r0 == beginning_row && r1 == r0 + direction * 2))
                && (c1 == c0));
    }    
    return false;
}
                    ...
                    int & r1, int & c1)
{
    while (1)
    {
        ...
        if (chessboard[r1][c1] != ' ' &&
            !is_enemy_piece(chessboard, player, r1, c1))
        {
            std::cout << "you have to put the piece at space/enemy spot\n";
            continue;            
        }
        if (lowercase(chessboard[r0][c0]) == 'p')
        {
            if (!is_valid_pawn_move(chessboard, player, r0, c0, r1, c1))
            {
